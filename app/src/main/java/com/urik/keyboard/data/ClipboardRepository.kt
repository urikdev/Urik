...
@Inject
constructor(private val clipboardDao: ClipboardDao) {
    suspend fun addItem(content: String): Result<Unit> = try {
        val item =
            ClipboardItem(
                content = content,
    ...
    suspend fun getPinnedItems(): Result<List<ClipboardItem>> = try {
    ...
    suspend fun togglePin(id: Long, pinned: Boolean): Result<Unit> = try {
    ...
    suspend fun deleteAllUnpinned(): Result<Unit> = try {
