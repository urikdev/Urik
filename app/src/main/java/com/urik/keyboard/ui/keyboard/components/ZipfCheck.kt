...
class ZipfCheck
@Inject
constructor(private val spellCheckManager: SpellCheckManager) {
    data class ArbitrationResult(val candidates: List<WordCandidate>, val arbiterFired: Boolean, val winReason: String)
    ...
    private fun disambiguateCloseCompetitors(
        ...
        val originalLeaderBonus1 = if (candidate1.combinedScore >
            candidate2.combinedScore
        ) {
            0.02f * leaderScale
        } else {
            0f
        }
        val originalLeaderBonus2 = if (candidate2.combinedScore >
            candidate1.combinedScore
        ) {
            0.02f * leaderScale
        } else {
            0f
    ...
    private fun determineWinReason(
        ...
        if (candidate1Won && bigramBoost1 > 0f || !candidate1Won && bigramBoost2 > 0f) {
            ...
            if (candidate1Won &&
                withoutBigram1 < withoutBigram2 ||
                !candidate1Won &&
                withoutBigram2 < withoutBigram1
            ) {
                return "bigram"
            }
        }
        ...
        if (freqWeight >= 0.50f) {
            ...
            if (candidate1Won &&
                freqContrib1 > freqContrib2 ||
                !candidate1Won &&
                freqContrib2 > freqContrib1
            ) {
                return "frequency"
            }
        }
