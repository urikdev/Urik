...
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    fun updateSwipe(point: PointF) {
        ...
        if (pathPointCount > 0) {
            ...
            if (distanceSquared > MIN_DISTANCE_THRESHOLD_SQUARED && pathPointCount < MAX_PATH_POINTS) {
                currentPoint.set(point.x, point.y)
                hasCurrentPoint = true
    ...
    private fun calculateDistanceSquared(x1: Float, y1: Float, x2: Float, y2: Float): Float {
        val dx = x2 - x1
        val dy = y2 - y1
        return dx * dx + dy * dy
    }
