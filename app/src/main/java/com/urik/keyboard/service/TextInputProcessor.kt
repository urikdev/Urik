...
constructor(
    ...
    suspend fun processWordInput(word: String, inputMethod: InputMethod = InputMethod.SWIPED): ProcessingResult =
    ...
    private suspend fun processWordInternal(word: String, inputMethod: InputMethod): ProcessingResult {
        try {
    ...
    suspend fun getSuggestions(word: String): List<SpellingSuggestion> = withContext(Dispatchers.Default) {
    ...
    suspend fun validateWord(word: String): Boolean = withContext(Dispatchers.Default) {
    ...
    private fun isValidWordInput(word: String): Boolean = word.isNotBlank() && word.length <= MAX_WORD_INPUT_LENGTH
    ...
    private fun cacheProcessing(word: String, normalized: String, graphemeCount: Int) {
        processingCache.put(
            word,
            ProcessingCache(
    ...
    private fun cacheSuggestions(word: String, suggestions: List<SpellingSuggestion>, isValid: Boolean) {
        suggestionCache.put(
            word,
