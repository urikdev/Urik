...
data class ExportResult(val wordCount: Int, val languages: List<String>)

data class ImportResult(val newWords: Int, val updatedWords: Int, val totalProcessed: Int)
...
constructor(
    ...
    suspend fun importFromUri(uri: Uri): Result<ImportResult> = withContext(ioDispatcher) {
        try {
            val jsonString =
