...
private data class VariationErrorEntry(val count: Int, val lastErrorTime: Long)
 ...
 */
private class BoundedVariationErrorTracker(private val maxSize: Int) {
    private val entries =
        object : LinkedHashMap<String, VariationErrorEntry>(maxSize + 1, 0.75f, true) {
            override fun removeEldestEntry(eldest: MutableMap.MutableEntry<String, VariationErrorEntry>?): Boolean =
                size > maxSize
        }
...
constructor(
    ...
    private fun loadVariationsFromAssets(context: Context, languageCode: String): Map<String, List<String>> {
        ...
        return context.assets.open("characters/$filename").bufferedReader().use { reader ->
            ...
            if (jsonContent.isBlank()) {
                error("Character variations file $filename is empty")
            }
    ...
    private fun parseVariationsJson(jsonContent: String): Map<String, List<String>> {
        ...
        if (result.isEmpty()) {
            error("No valid character variations found in file")
        }
    ...
    private fun getMinimalFallbackVariations(): Map<String, List<String>> = mapOf(
