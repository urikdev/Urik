...
class ClipboardDaoTest {
    ...
    @Test
    fun `insert adds item and returns id`() = runTest {
    ...
    @Test
    fun `insert with OnConflictStrategy IGNORE skips duplicate`() = runTest {
    ...
    @Test
    fun `getRecentItems returns unpinned sorted by timestamp DESC`() = runTest {
    ...
    @Test
    fun `getPinnedItems returns pinned sorted by timestamp DESC`() = runTest {
    ...
    @Test
    fun `updatePinned toggles pin status`() = runTest {
    ...
    @Test
    fun `updatePinned unpins item`() = runTest {
    ...
    @Test
    fun `deleteAllUnpinned removes only unpinned items`() = runTest {
    ...
    @Test
    fun `getCount returns zero for empty table`() = runTest {
        assertEquals(0, dao.getCount())
    }

    @Test
    fun `insert handles long content`() = runTest {
    ...
    @Test
    fun `getRecentItems handles empty table`() = runTest {
        val results = dao.getRecentItems()
    ...
    @Test
    fun `getPinnedItems handles empty table`() = runTest {
        val results = dao.getPinnedItems()
    ...
    @Test
    fun `timestamp ordering is stable`() = runTest {
    ...
    @Test
    fun `pin and unpin workflow`() = runTest {
        val item = createTestItem("Test")
        val id = dao.insert(item)
    ...
    @Test
    fun `getUnpinnedCount returns correct count`() = runTest {
        dao.insert(createTestItem("Unpinned1", isPinned = false))
        dao.insert(createTestItem("Unpinned2", isPinned = false))
        dao.insert(createTestItem("Pinned1", isPinned = true))
        dao.insert(createTestItem("Pinned2", isPinned = true))
    ...
    @Test
    fun `enforceMaxItems keeps only newest unpinned items`() = runTest {
    ...
    @Test
    fun `enforceMaxItems does not delete pinned items`() = runTest {
        dao.insert(createTestItem("Pinned1", timestamp = 100, isPinned = true))
        dao.insert(createTestItem("Pinned2", timestamp = 200, isPinned = true))
    ...
    @Test
    fun `getRecentItems respects limit parameter`() = runTest {
        dao.insert(createTestItem("First", timestamp = 100, isPinned = false))
        dao.insert(createTestItem("Second", timestamp = 200, isPinned = false))
        dao.insert(createTestItem("Third", timestamp = 300, isPinned = false))
